...
Window {
    ...
    Component.onCompleted: source = "content://com.android.externalstorage.documents/document/primary%3AAudio%2FWavKaylen3.wav"
    Popup {
        id: mediaError
    ...
    function read(metadata) {
      const dic = {}
        for (const key of metadata.keys()) {
          const value = metadata.stringValue(key);
          if (value) {
            dic[metadata.metaDataKeyToString(key)] = value;
          }
        }
    ...
    MediaPlayer {
        ...
        function updateMetadata() {
            ...
            console.log("ddddd", mediaPlayer.source)
        }
        onPlaybackStateChanged: console.log("ddddd", JSON.stringify(read(metaData)))
